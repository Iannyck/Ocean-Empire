...
{
    [HideInInspector] public Func<GameObject,Vector2, GameObject> spawnFunc;
    ...
    private PoolableUnit CreateNewCopy(PoolableUnit prefab, Vector2 position)
    {
        if(spawnFunc == null)
        {
            Debug.LogError("Need a spawn func");
            return null;
        }
        else
        {
            var copy = spawnFunc(prefab.gameObject, position).GetComponent<PoolableUnit>();
