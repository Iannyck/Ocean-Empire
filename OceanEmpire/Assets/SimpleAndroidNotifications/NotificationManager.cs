...
using System.Linq;
    ...
    public static class NotificationManager
    {
        ...
        /// </summary>
        /// <param name="smallIcon">List of build-in small icons: notification_icon_bell (default), notification_icon_clock, notification_icon_heart, notification_icon_message, notification_icon_nut, notification_icon_star, notification_icon_warning.</param>
        public static int SendWithAppIcon(TimeSpan delay, string title, string message, Color smallIconColor, NotificationIcon smallIcon = 0, int id = -1)
        {
        if (id == -1)
        {
            id = UnityEngine.Random.Range(0, int.MaxValue);
        }
        NotificationParams notif = new NotificationParams
        {
            Id = id,
            Delay = delay,
            Title = title,
